...
@Component({
  selector: 'app-dmsinvoice-list',
  templateUrl: './dmsinvoice-list.component.html',
  styleUrls: ['./dmsinvoice-list.component.scss']
})
...
export class DmsinvoiceListComponent extends BaseComponent implements OnInit, AfterViewInit {
     ...
     --,it.[DmscInvoiceAndReceiptStatus]
            ...
            try {
              ...
              for (let el of response.data.response ) {
                ...
                const url = 'http://192.168.200.28/PaymentService/ImportInvoiceInfo';
                //const url = 'https://ilabplusbackend.dmsc.moph.go.th/PaymentService/ImportInvoiceInfo';
                const headers = new HttpHeaders().set('Content-Type', 'application/json')
                const body =
                {
                  //"User": "test",
                  //"Token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJlYjVmYjk4YS1lNzcxLTQ3OTYtYjczMS1lOWMwNGU1OGE4ZWQiLCJ2YWxpZCI6IjEiLCJodHRwOi8vc2NoZW1hcy54bWxzb2FwLm9yZy93cy8yMDA1LzA1L2lkZW50aXR5L2NsYWltcy9uYW1lIjoidGVzdCIsImV4cCI6MTY4ODUyNzk3MiwiaXNzIjoiRE1TQ19QYXltZW50IiwiYXVkIjoiRE1TQ19QYXltZW50In0.u0sARPi_vwsUZpP6Kzrgahpd27Zk9aiBqdmay7o9V7g",
                  "User": "iem_user",
                  ...
                  "Data": {
                    ...
                    "InvoiceItems": JSON.parse(jsonString),
                    "Customer": {
                ...
                try {
                  this.http.post(url, body, { observe: 'response' }).pipe(
                    ...
                    (response: HttpResponse<any>) => {
                      ...
                      if (response.body.Status == "01") {

                        for (let el of this.selection.selected) {

                          const query = `
                         UPDATE  Requests
                         SET dmscInvoiceAndReceiptStatus = 'SentAPI_Invoice'
                         WHERE requestID = '${el.requestID}'
                        `;
                          const response = this.sentSampleService.query({ queryString: query });
                        }
                        ...
                        this.getData();
                        this.spinner.hide();
                        this.toastrNotiService.success('ส่งใบแจ้งหนี้ Invoice เรียบร้อยแล้ว', 'Message');
                        ...
                        for (let el of this.selection.selected) {
                          const query = `
                         Update  Requests
                         Set DmscInvoiceAndReceiptStatus = 'CancelAPI_Invoice'
                         WHERE requestID = '${el.requestID}'
                        `;
                          const response = this.sentSampleService.query({ queryString: query });
                        }
                        ...
                        //const urlCheckStatus = 'https://ilabplusbackend.dmsc.moph.go.th/PaymentService/ImportInvoiceCheckStatus';
                        const urlCheckStatus = 'http://192.168.200.28/PaymentService/ImportInvoiceCheckStatus';
                  ...
                  try {
                    this.http.post(url, body, { observe: 'response' }).pipe(
                      ...
                      (response: HttpResponse<any>) => {
                        ...
                        var convertedString = JSON.stringify(response)
                        //console.log('SendAPI => ', response.body.Status);
                        if (response.body.Status == "01") {
                        ...
                        `;
                          ...
                          const urlCheckStatus = 'https://ilabplusbackend.dmsc.moph.go.th/PaymentService/ImportInvoiceCheckStatus';
                          //const urlCheckStatus = 'http://192.168.200.28/PaymentService/ImportInvoiceInfo';
                          const headersCheckStatus = new HttpHeaders().set('Content-Type', 'application/json')
                          const bodyCheckStatus =
                          {
                            "User": "test",
